...
class Book {
    ...
    private:
        ...
        void marketBuy(Order &order, std::vector<OrderEvent> &responses, const int &time);

        void marketSell(Order &order, std::vector<OrderEvent> &responses, const int &time);

        std::optional<Order> try_remove(const int &ID_ToBeRemoved, const int &owner_id);

        std::optional<Order> removeFromSide(std::map<int, Location_Rest> &Level, const int &price, const int &owner_id,
        ...
        void matchBuy(Order &order, std::vector<OrderEvent> &responses, const int &time);

        void matchSell(Order &order, std::vector<OrderEvent> &responses, const int &time);

        OrderEvent Fill(const int &order_id, const int &owner_id, const Side &side, const int &remaining_quantity,
