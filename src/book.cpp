...
std::vector<OrderEvent> Book::ProcessAdd(const BackTestEvent &Event) {
    ...
    if (location.find(Event.order_id) != location.end()) {
        ...
        return {{Event.side, ResponseEvents::OrderRejected, Event.order_id, Event.owner_id, Event.quantity, Event.price,
                 Event.time}};
    }

    std::vector<OrderEvent> responses = {{Event.side, ResponseEvents::OrderAccepted, Event.order_id, Event.owner_id,
...
std::vector<OrderEvent> Book::ProcessCancel(const BackTestEvent &Event) {
    ...
    if (!removed.has_value()) {
        ...
        return {{Event.side, ResponseEvents::OrderCancelFailed, Event.order_id, Event.owner_id, Event.quantity,
                 Event.price, Event.time}};
    }

    return {{removed->side, ResponseEvents::OrderCancelled, removed->order_id, removed->owner_id, removed->quantity,
             removed->price, Event.time}};
}
...
std::vector<OrderEvent> Book::ProcessMod(const BackTestEvent &Event) {
    ...
    if (Event.quantity <= 0 || Event.price <= 0) {
        ...
        return {{Event.side, ResponseEvents::OrderModifyFailed, Event.order_id, Event.owner_id, Event.quantity,
                 Event.price, Event.time}};
    }

    auto locationIt = location.find(Event.order_id);

    if (locationIt == location.end() || locationIt->second.owner_id != Event.owner_id ||
...
std::vector<OrderEvent> Book::ProcessIOC(const BackTestEvent &Event) {
    ...
    if (Event.quantity <= 0 || Event.price <= 0 || location.find(Event.order_id) != location.end()) {
    ...
    if (order_data.quantity > 0) {
        ...
        responses.push_back({Event.side, ResponseEvents::OrderExpired, Event.order_id, Event.owner_id,
                             order_data.quantity, Event.price, Event.time});
    }
...
void Book::marketBuy(Order &order, std::vector<OrderEvent> &responses, const int &time) {
...
void Book::marketSell(Order &order, std::vector<OrderEvent> &responses, const int &time) {
    ...
    while (!bids.empty() && order.quantity > 0) {
        ...
        while (order.quantity > 0 && it != NULL) {
            ...
            responses.push_back(Fill(it->order_id, it->owner_id, Side::Buy, it->quantity, delta, price, time));
...
std::optional<Order> Book::try_remove(const int &ID_ToBeRemoved, const int &owner_id) {
...
std::optional<Order> Book::removeFromSide(std::map<int, Location_Rest> &Level, const int &price, const int &owner_id,
...
void Book::init_level(const Order &order, std::map<int, Location_Rest> &Levels) {
...
void Book::add_order(const Order &order, std::map<int, Location_Rest> &Levels) {
    ...
    LevelNode *location_Ptr =
        new LevelNode{NULL, Levels[order.price].last, order.order_id, order.owner_id, order.quantity};
...
void Book::matchBuy(Order &order, std::vector<OrderEvent> &responses, const int &time) {
...
void Book::matchSell(Order &order, std::vector<OrderEvent> &responses, const int &time) {

    while (!bids.empty() && bids.rbegin()->first >= order.price && order.quantity > 0) {
...
OrderEvent Book::Fill(const int &order_id, const int &owner_id, const Side &side, const int &remaining_quantity,
                      const int &traded_quantity, const int &price, const int &time) {
