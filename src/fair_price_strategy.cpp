...
std::optional<OrderCommand> FairPriceStrategy::onTimeMove(int now, const std::map<Symbol, Book> &books,
    ...
    double imbalance =
        static_cast<double>(top->best_bid_quantity - top->best_ask_quantity) / static_cast<double>(totalQuantity);

    double mid = (static_cast<double>(top->best_bid) + static_cast<double>(top->best_ask)) / 2.0;
    ...
    double requiredBuyEdge = config_.baseEdge + position * config_.inventoryPenalty;

    double requiredSellEdge = config_.baseEdge - position * config_.inventoryPenalty;

    if (position < config_.maxPosition && buyEdge >= requiredBuyEdge) {

        return OrderCommand{config_.symbol, OrderType::Add, 0, Side::Buy, 1, top->best_ask};
    }

    if (position > -config_.maxPosition && sellEdge >= requiredSellEdge) {

        return OrderCommand{config_.symbol, OrderType::Add, 0, Side::Sell, 1, top->best_bid};
    }
