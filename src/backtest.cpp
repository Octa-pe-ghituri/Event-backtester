...
Backtest::Backtest(std::string eventsPath, std::unique_ptr<Strategy> strategy, int maxTime, int strategyLatency,
    ...
    for (const BackTestEvent &event : LoadEventsFromFile(eventsPath)) {
        if (event.owner_id == strategyOwnerId_) {
            throw std::invalid_argument("historical event uses reserved strategy owner id");
        }
...
void Backtest::printSummary() const {
    ...
    for (const auto &entry : portfolio_.positions()) {
        ...
        double mark = (static_cast<double>(top->best_bid) + static_cast<double>(top->best_ask)) / 2.0;
...
const std::map<Symbol, Book> &Backtest::books() const {
    return books_;
}
...
void Backtest::scheduleCommand(const OrderCommand &command) {
    ...
    bool isNewOrder = command.order_type == OrderType::Add || command.order_type == OrderType::IOC ||
    ...
    BackTestEvent event{command.symbol,   command.side,     command.order_type, orderId,
                        strategyOwnerId_, command.quantity, command.price,      now_ + strategyLatency_};
